...
We will structure the backend using relational tables, utilizing Clerk for authentication. The core `User` model will be keyed directly to the `clerk_id` generated by Clerk.

### Table 1: User
- `clerk_id` (String, PK - Linked to Clerk Auth)
- `created_at` (DateTime)

### Table 2: GoalTracker *(New)*
- `id` (UUID, PK)
- `clerk_user_id` (FK -> User)
